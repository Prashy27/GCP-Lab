data "template_file" "inventory_gcp"{
    ...
    vars = {
        master_ip = "${google_compute_instance.kubemaster.network_interface.access_config.nat_ip}"
        # master_ip = "${master_if.*.nat_ip}"
        worker_ip = "${google_compute_instance.kubenode02.network_interface.access_config.nat_ip}"
